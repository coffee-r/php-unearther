...
php-unearthは、CodeIgniter3アプリケーションの実行時の振る舞いを **JSONLログ** として記録し、人間や生成AIが安全に分析できる証拠データセットへ変換するPHPライブラリを目指します。

このpackageは、endpointをそのまま移行単位だと決めつけません。CodeIgniter3 Appでは、移行前の1 endpointが移行先で複数APIに分割されたり、複数endpointが1つのAPIへ統合されたり、一部の責務だけが別APIへ移ることがあります。php-unearthは、まず実行時に起きたSQL、外部API、request/response、view、errorを同じtraceで集め、後から業務仕様候補や移行対応関係を考えるための材料を作ります。

php-unearthが直接「仕様書を完成させる」わけではありません。移行前コード、観測ログ、redacted export、RAG / LLM、人間の確認を組み合わせて、根拠付きの日本語仕様候補を作るための土台を提供します。
...
php-unearthは、これらを「実際に動いた事実」から確認できるようにします。
...
汎用APMの代替ではありません。主目的は仕様発掘と、移行先実装でも同じformatのログを出して照合できる土台を作ることです。高度な分析、要約、pattern名付け、日本語仕様化、比較report、移行対応関係の解釈は生成AIや外部ツールや人間に委ねられるので、php-unearth本体は **収集・正規化・マスキング・export・簡易集計** に寄せます。
...
ここでの入口は、移行単位そのものではありません。旧WebAppの`/users/{id}/point`が移行先の`/users/{id}`へ統合されることもあれば、旧WebAppの`/carts/{id}`の一部が移行先の`/carts/{id}/point-items`へ分割されることもあります。php-unearthは、そうした移行設計を決定するための観測事実を作ります。
...
* Git repository: `php-unearth`
* Composer package: `coffee-r/php-unearth`
* CLI command: `unearth`
* PHP namespace: `CoffeeR\Unearth`

例:

```bash
composer require coffee-r/php-unearth
...
```json
{
  "repositories": [
    {
      "type": "vcs",
      "url": "https://github.com/coffee-r/php-unearth"
    }
  ],
  "require": {
    "coffee-r/php-unearth": "dev-main"
  }
}
```
...
```bash
composer update coffee-r/php-unearth
```
...
php-unearthは、サンプリングされたリクエストごとに1つのJSONオブジェクトを書き出します。
...
実行時間/レイテンシは意図的に記録していません。php-unearthはエンドポイントが観測可能な動作を明らかにするためのもので、速度の測定用ではありません。
...
php-unearthが出すログは「実際に観測できたリクエストの証拠」であり、CodeIgniter3 App全体の仕様を100%復元するものではありません。サンプリング対象にならなかったリクエスト、テストしていない入力パターン、夜間バッチ、外部システム側の非同期処理、DB trigger、共有DBやsession DBを介した他Appの変更は、同じtraceに必ず現れるとは限りません。
...
v1ではsession差分の記録を対象外にします。CI3のsession保存先がDBやRedisなどの場合でも、php-unearthはsession payloadのbefore/afterを自動比較しません。session DBへのSQLが観測される可能性はありますが、それは「このrequest内で発行されたSQL」の証拠であり、session上の業務メッセージを完全に復元するものではありません。
...
php-unearthが出すのは、仕様そのものではなく「観測された事実」です。

「商品代金が7000円以上なら送料無料」「支払い代金が100000円以上だと後払いが選べない」のような日本語仕様は、観測ログだけで常に断定できるわけではありません。実際には、移行前コード、設定、DBマスタ、外部API、検証環境で流したシナリオ、観測されたresponse差分を組み合わせて判断します。php-unearthは、その判断に使う証拠を安全に集め、RAG / LLMや人間に渡しやすい形へ整えます。
...
本番環境や本番相当の環境でphp-unearthを有効にする際は注意が必要です。生のリクエスト・レスポンス値を保存しない場合でも、観測ログには機密情報が含まれる可能性があります。
...
初期プロトタイプにはマスキングポリシーエンジンは含まれていません。観測フィールドにシークレット、トークン、メールアドレス、氏名、住所、電話番号、決済識別子、顧客識別子が含まれる可能性がある場合は、アプリケーション側でフィルタリングを追加するか、キャプチャ範囲が明確になるまでphp-unearthを無効のままにしてください。
...
`failure_mode`は、php-unearth自体がリクエスト観測中に失敗した場合の動作を制御します。デフォルトは`log`で、観測失敗時もアプリケーション本体を継続させます。`log`モードでは、CodeIgniter3は利用可能な場合`log_message('error', ...)`を使用し、それ以外は`error_log()`にフォールバックします。stagingなどでは`throw`に上げると、インストールや設定の問題をロールアウト中に可視化できます。

`http.endpoint_patterns`はオプションです。リクエストが`/api/users/{id}`のようなプレースホルダー付きパスパターンにマッチした場合、観測の`path_pattern`がその正規化された形式に設定され、レポートがそれでグループ化されます。php-unearthはパスパラメーターを自動推定しません；設定されていないパスは`path_pattern`が生のリクエストパスと同じ値になります。
...
CodeIgniter3はhook定義内でnamespaceつきComposerクラスをネイティブに前提としていません。最も安全なセットアップは、アプリケーション内に小さなブリッジhookを作成し、そこからphp-unearthを呼び出す方法です。
...
リクエストのContent-Typeが`application/json`または構造化された`+json`タイプの場合、php-unearthはボディをデコードしてJSONの形状を記録します。ボディが無効なJSON、サイズ超過、またはJSON以外の場合は`$_POST`の形状にフォールバックします。

JSONリクエスト形状のキャプチャは`php://input`を読み取ります。サポートされているPHPランタイムでは通常再読み取り可能ですが、フックが実行される前にアプリケーションのブートストラップコードが生の入力を消費または置き換えた場合、php-unearthはJSONボディを読み取れず`$_POST`にフォールバックします。
...
パスに`{date}`が含まれない場合、php-unearthはそのパスにそのまま書き込みます。
...
report / exportには10MBなどの固定サイズ上限は設けません。大きなログを生成AIや外部ツールへ渡す場合は、利用者側で期間、endpoint、環境、sample file単位に入力JSONLを分割してください。php-unearthは、渡された入力に対して決定論的にredacted export / aggregate reportを生成します。
...
PHPでは一部のケースで関数引数をcaptureできますが、php-unearthでは広範な自動引数captureには依存しません。
...
php-unearthはcaller file / lineの自動取得を行いません。CodeIgniter3のroute/controller情報と観測されたSQL/API patternを主な証拠として扱います。raw request bodyも無条件には保存しません。

## APM / OpenTelemetryとの関係

php-unearthはAPMやOpenTelemetryの概念と重なる部分がありますが、目的が違います。
...
* php-unearth本体が仕様を断定生成するのではなく、仕様候補を作るための根拠を出す、という境界がREADMEで明確になっている
...
```bash
docker compose -f tests/e2e/codeigniter3-sqlite/docker-compose.yml -p php-unearth-ci3-sqlite exec -T --user www-data app php scripts/init-db.php
