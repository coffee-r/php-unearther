...
CodeIgniter3 Appの実行時の振る舞いを掘り起こし、仕様調査と移行調査に使うための観測ツール。

php-ci3-unearthは、CodeIgniter3アプリケーションの実行時の振る舞いを **JSONLログ** として記録し、人間や生成AIが安全に分析できる証拠データセットへ変換するPHPライブラリを目指します。

このpackageは、endpointをそのまま移行単位だと決めつけません。CodeIgniter3 Appでは、移行前の1 endpointが移行先で複数APIに分割されたり、複数endpointが1つのAPIへ統合されたり、一部の責務だけが別APIへ移ることがあります。php-ci3-unearthは、まず実行時に起きたSQL、外部API、request/response、view、errorを同じtraceで集め、後から業務仕様候補や移行対応関係を考えるための材料を作ります。

php-ci3-unearthが直接「仕様書を完成させる」わけではありません。移行前コード、観測ログ、redacted export、RAG / LLM、人間の確認を組み合わせて、根拠付きの日本語仕様候補を作るための土台を提供します。
...
移行先はLaravelやC#などさまざまでも構いませんが、このpackage自体はCodeIgniter3専用の観測器として割り切ります。移行先側のadapterや比較器は本packageの責務に含めません。

本ツールはCodeIgniter3専用の観測器として設計しています。CI3は現在もLTSとしてsecurity patchが提供されており、本ツールはそうしたシステムを移行する際の調査作業を支援することを目的としています。

このパッケージは初期プロトタイプです。

## なぜ作るか

CodeIgniter3 Appの移行では、既存仕様が十分に文書化されていないことが多くあります。
...
* 外部APIを呼び出しているのか
* カート、顧客、商品、注文、支払いなどの業務domainごとに、実際にはどんな分岐があるのか
* 移行前のendpoint単位と移行先のAPI単位が、1:1、N:M、分割、統合、部分移動のどれに近いのか
* 生成AIに分析させたいが、raw値をそのまま渡すのは危険ではないか

php-ci3-unearthは、これらを「実際に動いた事実」から確認できるようにします。

例えば、次のような調査を支援することを目指します。

* 送料適用APIを実行したときに流れるSQLと、その分岐パターンを知る
* カート画面に遷移したときに呼ばれる外部APIと、その呼び出しパターンを知る
* 「商品代金が7000円以上なら送料無料、それ以外は590円」のような仕様候補を、観測根拠付きで日本語化する
* 「支払い代金が100000円以上だと後払いが選べない」のような業務ルール候補について、コード上の分岐と観測ログの両方を根拠として確認する
* 顧客、商品、注文、カート、支払いなどのdomain単位で、移行前WebAppの振る舞いを整理する

汎用APMの代替ではありません。主目的は仕様発掘と、移行先実装でも同じformatのログを出して照合できる土台を作ることです。高度な分析、要約、pattern名付け、日本語仕様化、比較report、移行対応関係の解釈は生成AIや外部ツールや人間に委ねられるので、php-ci3-unearth本体は **収集・正規化・マスキング・export・簡易集計** に寄せます。

## 主目的

既存のCodeIgniter3アプリケーション (API / MVCモノリスの両方) から新しい実装へ移行するときに、仕様調査にかかる時間と不確実性を減らします。
...
* SQL呼び出し
* 外部HTTP呼び出し
* view/templateとview変数shape
* error

目指す状態は、developerが観測された入口を見たときに、次のようなことを確認できることです。

> この入口は、こういう入力で使われ、こういう出力を返し、このテーブルを触り、このSQLパターンで分岐し、この外部APIを呼んでいる。

ここでの入口は、移行単位そのものではありません。旧WebAppの`/users/{id}/point`が移行先の`/users/{id}`へ統合されることもあれば、旧WebAppの`/carts/{id}`の一部が移行先の`/carts/{id}/point-items`へ分割されることもあります。php-ci3-unearthは、そうした移行設計を決定するための観測事実を作ります。

このREADMEでは、以下の用語を使います。

* observed flow: 1 requestで実際に観測されたHTTP入出力、SQL、外部API、view、errorの流れ
* behavior pattern: 同じ入口で観測されたSQL / 外部API / status / errorなどの分岐パターン
* migration evidence: 旧WebAppの振る舞いを移行先実装へ対応付けるための根拠

そしてもう一つの軸として、 **CodeIgniter3から安全にAIへ渡せるJSONL schemaを固める** ことを重視します。移行確認や新実装側のログ生成は、人間 / 生成AI / 外部ツール側に委ねます。
...
* observed entrypoint別に、最低限のJSON / Markdown集計を出す
* RAG / LLM投入前の材料として使える安全なsubsetを出す
...
* pattern名付け
* 既存実装と新実装の自動比較
* 旧endpointと新APIの完全自動対応付け
* RAG専用chunk dataset export
* HTML UI / HTML reportの作り込み
* Mermaid sequence diagramの自動生成
* 完全なcall graph
* 観測されなかった入力値、分岐、非同期処理、外部システム状態を含む100%のデータ掘り起こし

## Schema仕様と検証

JSONLの構造はコードを読まなくても分かる場所に置きます。
...
* Git repository: `php-ci3-unearth`
* Composer package: `coffee-r/php-ci3-unearth`
* CLI command: `unearth`
* PHP namespace: `CoffeeR\Ci3Unearth`

例:

```bash
composer require coffee-r/php-ci3-unearth
...
Laravel 10以降やC#などの移行先adapterはこのpackageでは扱いません。まずはCodeIgniter3で実データを安全に集め、移行前仕様を掘り起こすための証拠データセットを作ることに集中します。
...
* RAG専用chunk dataset export
* domain tagging / scenario metadata / migration mapping metadata
* 生成AIによる仕様推論や日本語仕様化

設計は特定のアプリケーション名に閉じません。ただし、v1実装はCodeIgniter3専用にします。

scope削減の判断:

* Laravel adapter実装そのものは削る。CodeIgniter3観測に集中する。
* HTML report、Mermaid diagram、OpenAPI補助情報、自動比較、RAG専用chunk exportはv1から削る。
...
CodeIgniter3以外のレガシーフレームワーク、ノンフレームワークPHP、batch / CLI、Laravel adapter、C# adapterなどは追いません。本packageはCodeIgniter3の移行調査に必要な観測データを作ることだけを担います。
...
```json
{
  "repositories": [
    {
      "type": "vcs",
      "url": "https://github.com/coffee-r/php-ci3-unearth"
    }
  ],
  "require": {
    "coffee-r/php-ci3-unearth": "dev-main"
  }
}
```
...
```bash
composer update coffee-r/php-ci3-unearth
```
...
```bash
cp -R vendor/coffee-r/php-ci3-unearth/resources/codeigniter3/application/hooks application/
cp vendor/coffee-r/php-ci3-unearth/resources/codeigniter3/application/config/unearth.php application/config/
```
...
```bash
cp vendor/coffee-r/php-ci3-unearth/resources/codeigniter3/application/core/Unearth_Loader_Trait.php application/core/
cp vendor/coffee-r/php-ci3-unearth/resources/codeigniter3/application/core/MY_Loader.php application/core/
```
...
```bash
vendor/bin/unearth export application/logs/unearth-*.jsonl --profile ai --format jsonl
vendor/bin/unearth report application/logs/unearth-*.jsonl --format markdown --value-mode normalized
```
...
php-ci3-unearthは、サンプリングされたリクエストごとに1つのJSONオブジェクトを書き出します。

各観測レコードには`schema_version`フィールドが含まれます。現在のスキーマバージョンは`1`です。

観測レコードに含まれる情報：

- HTTPメソッド、生の`path`、正規化された`path_pattern`、`route`、`controller`、`action`、`controller_path`、ステータス、`response_kind`（`json` / `html` / `other`）、`query_shape`、`query_raw`（デフォルトnull）、`request_shape`、`request_raw`（デフォルトnull）、`response_shape`
- SQLの操作種別、`tables`、`statement_normalized`（リテラルを`{parameter}`に置換）、`statement_text`（生SQL、オプトイン；それ以外は`null`）、`statement_hash`（`statement_normalized`のsha256）、`bind_shape`
- Guzzleミドルウェア経由で記録された外部HTTP呼び出し（ホスト、メソッド、パス、ステータス）
- アダプターまたはラッパーが記録したエラー

可能な場合、値は「形状（shape）」として表現されます。例えばリクエストフィールドは生の値ではなく、`string`、`number`、`boolean`、`array`、またはネストされた構造として記録されます。

実行時間/レイテンシは意図的に記録していません。php-ci3-unearthはエンドポイントが観測可能な動作を明らかにするためのもので、速度の測定用ではありません。

`calls`やSQL callerは記録しません。AIに渡す主導線は、観測入口、CI3 controller path、request/response shape、SQL/API pattern、table catalogに寄せます。
...
reportでは、table抽出を「観測された確定事実」として強く言い切りません。特に `tables_confidence` が `best_effort` / `unknown` の場合は、SQL flowや参照/更新テーブルの表示に注記を付けます。移行調査で最終確認が必要な場合は、`controller_path`、`statement_normalized`、`statement_hash`、必要に応じてraw SQLを見て人間が確認します。
...
php-ci3-unearthが出すログは「実際に観測できたリクエストの証拠」であり、CodeIgniter3 App全体の仕様を100%復元するものではありません。サンプリング対象にならなかったリクエスト、テストしていない入力パターン、夜間バッチ、外部システム側の非同期処理、DB trigger、共有DBやsession DBを介した他Appの変更は、同じtraceに必ず現れるとは限りません。

また、CI3 Appでは `customer_no` のような同じ概念の値が、画面、API、DB、session、外部連携の境界で `string` / `number` / `null` として揺れることがあります。shapeは単発の型断定ではなく、観測された範囲で型の候補を集めるための材料として扱います。同じキーに異なるスカラー型が観測された場合、shapeは `"string|number"` や `"null|string"` のようにパイプ区切りのunion表現で記録されます。最終的な型や必須/任意の判断は、観測ログ、コード、DB定義、実シナリオを突き合わせて人間が確認します。

### Session

v1ではsession差分の記録を対象外にします。CI3のsession保存先がDBやRedisなどの場合でも、php-ci3-unearthはsession payloadのbefore/afterを自動比較しません。session DBへのSQLが観測される可能性はありますが、それは「このrequest内で発行されたSQL」の証拠であり、session上の業務メッセージを完全に復元するものではありません。
...
Appへのin / App外へのoutは、まずHTTP request/responseとGuzzle out-bound HTTPを薄く取る方針です。一方で、別Appとの連携が共有DB、session DB、キュー、ファイル、メッセージテーブルなどを介している場合、通信境界がHTTPのように明確ではありません。v1ではそのメッセージ本文を汎用的に解釈せず、観測できたSQL patternや外部HTTP callを「連携の痕跡」として記録します。将来対応するなら、業務ごとのメッセージテーブル、queue producer/consumer、session storeなどに薄いadapterを足し、raw値を保存しないshape/token中心で扱うのが現実的です。
...
php-ci3-unearthが出すのは、仕様そのものではなく「観測された事実」です。

「商品代金が7000円以上なら送料無料」「支払い代金が100000円以上だと後払いが選べない」のような日本語仕様は、観測ログだけで常に断定できるわけではありません。実際には、移行前コード、設定、DBマスタ、外部API、検証環境で流したシナリオ、観測されたresponse差分を組み合わせて判断します。php-ci3-unearthは、その判断に使う証拠を安全に集め、RAG / LLMや人間に渡しやすい形へ整えます。
...
* しきい値や業務ルールは、観測値やコード断片が十分でない場合は「仕様候補」として扱う
...
観測ログを生成AIに要約させたい、pattern命名させたい、RAGに投入したい、別組織の人にreportを見せたいといった用途では、raw値が含まれていると個人情報・認証情報・決済情報の流出経路になります。
...
* このsubset生成はpackage側にCLIとして用意します (例: `unearth export --profile ai`)
...
```json
{
  ...
  "http": {
    ...
    "controller": "cart",
    "action": "add",
    "controller_path": "application/controllers/api/Cart.php",
    "status": 200,
    "response_kind": "json",
...
本番環境や本番相当の環境でphp-ci3-unearthを有効にする際は注意が必要です。生のリクエスト・レスポンス値を保存しない場合でも、観測ログには機密情報が含まれる可能性があります。

有効化前に以下を確認してください：

- エンドポイントパス、クエリキー、リクエストキー、レスポンスキー、SQLテキスト、bind shape、テーブル名、controller path、外部ホスト名などに個人情報・認証情報・テナント識別子・内部システム詳細が含まれていないか
...
初期プロトタイプにはマスキングポリシーエンジンは含まれていません。観測フィールドにシークレット、トークン、メールアドレス、氏名、住所、電話番号、決済識別子、顧客識別子が含まれる可能性がある場合は、アプリケーション側でフィルタリングを追加するか、キャプチャ範囲が明確になるまでphp-ci3-unearthを無効のままにしてください。
...
`failure_mode`は、php-ci3-unearth自体がリクエスト観測中に失敗した場合の動作を制御します。デフォルトは`log`で、観測失敗時もアプリケーション本体を継続させます。`log`モードでは、CodeIgniter3は利用可能な場合`log_message('error', ...)`を使用し、それ以外は`error_log()`にフォールバックします。stagingなどでは`throw`に上げると、インストールや設定の問題をロールアウト中に可視化できます。

`http.endpoint_patterns`はオプションです。リクエストが`/api/users/{id}`のようなプレースホルダー付きパスパターンにマッチした場合、観測の`path_pattern`がその正規化された形式に設定され、レポートがそれでグループ化されます。php-ci3-unearthはパスパラメーターを自動推定しません；設定されていないパスは`path_pattern`が生のリクエストパスと同じ値になります。
...
CodeIgniter3はhook定義内でnamespaceつきComposerクラスをネイティブに前提としていません。最も安全なセットアップは、アプリケーション内に小さなブリッジhookを作成し、そこからphp-ci3-unearthを呼び出す方法です。

`application/hooks/UnearthHook.php`を作成してください：
...
このため、query history由来のSQL eventには`analysis.warnings`として`query_history_capture_has_no_bind_values`が入ります。
...
サンプリングされたリクエストに対して、CodeIgniter3フックは`query_shape`と`request_shape`を記録します。query parameterは`$_GET`からshape化され、`redaction.secret`がある場合は`query_tokens`も記録します。raw query値は保存せず、`query_raw`は`null`のままです。

リクエストのContent-Typeが`application/json`または構造化された`+json`タイプの場合、php-ci3-unearthはボディをデコードしてJSONの形状を記録します。ボディが無効なJSON、サイズ超過、またはJSON以外の場合は`$_POST`の形状にフォールバックします。

JSONリクエスト形状のキャプチャは`php://input`を読み取ります。サポートされているPHPランタイムでは通常再読み取り可能ですが、フックが実行される前にアプリケーションのブートストラップコードが生の入力を消費または置き換えた場合、php-ci3-unearthはJSONボディを読み取れず`$_POST`にフォールバックします。

レスポンスボディの形状キャプチャはデフォルトでオンです。移行調査において `response_shape` はエンドポイントの出力仕様を確認するための重要な情報です。CodeIgniter3フックはJSONコンテンツタイプを持つサンプリングされたレスポンスに対して`$CI->output->get_output()`を読み取り、レスポンスの形状のみを記録します。

レスポンスに機密情報が含まれる場合や、取得を無効化したい場合は明示的にオフにしてください：
...
```php
'sink_path' => APPPATH . 'logs/unearth-{date}.jsonl'
```

これにより以下のようなファイルが書き出されます：

```text
unearth-2026-06-01.jsonl
unearth-2026-06-02.jsonl
```

パスに`{date}`が含まれない場合、php-ci3-unearthはそのパスにそのまま書き込みます。
...
$stack->push(UnearthMiddleware::create($collector));
```
...
簡易レポートは、一般的なAPI仕様書というより、実際に観測された入口の挙動を機械的に集計したものに留めます。特に、同じ入口内にあるSQL実行順や外部HTTP呼び出しの分岐パターンを見えるようにします。
...
vendor/bin/unearth report logs/*.jsonl --format md --value-mode normalized
...
vendor/bin/unearth report logs/*.jsonl --format markdown --table-catalog table-catalog.json
```
...
|---|---|---|
| `export --profile ai` | `jsonl`, `json` | raw値を除去したredacted observation |
| `report` | `md`, `markdown`, `json` | observed entrypoint別のaggregate report |

`jsonl` formatは`export`専用です。`report --format jsonl`はサポートしません。

`report`では任意でtable catalogを渡せます。table catalogはJSON objectで、keyをtable名、valueを一言説明にします。

```json
{
  "M_SHOHIN": "商品マスタ。商品コード、商品名、価格、販売状態を持つ。",
  "T_CART": "カート明細。顧客ごとの投入商品と数量を持つ。"
}
```

table catalogは自動推論しません。未指定のtableやcatalogに存在しないtableは、descriptionが空文字のままreportに出ます。
...
* Mermaid diagram
* OpenAPI補助情報
* RAG dataset / chunk export
* domain tagging metadata
* scenario metadata
...
AIを通さず、観測ログから決定論的に生成できるexport / reportを基本にします。AIによる要約、pattern名の意味づけ、業務ルール推論、日本語仕様化は、このpackage本体の責務にしません。

既存/新APIの比較は本packageでは扱いません。同じschemaのログが両側にあれば、人間 / 別ツール / 単純なdiffで比較できます。ただし、移行では旧endpointと新APIが1:1対応しないことが多いため、将来的にはdomain tag、scenario metadata、migration mapping metadataを別レイヤーで持てるようにする余地を残します。

report / exportには10MBなどの固定サイズ上限は設けません。大きなログを生成AIや外部ツールへ渡す場合は、利用者側で期間、endpoint、環境、sample file単位に入力JSONLを分割してください。php-ci3-unearthは、渡された入力に対して決定論的にredacted export / aggregate reportを生成します。

レポートはobserved entrypointと観測された実行パターンでトレースをグループ化します。現行実装では、observed entrypointはHTTP methodと`path_pattern`を中心にした集計単位です。これは移行単位そのものではなく、観測事実を読むための入口別ビューです。パターンは現在、SQLの操作/テーブル/ハッシュのフローとGuzzle外部HTTP呼び出しに基づいています。パターンの粒度はこのプロトタイプでは固定されており、設定できません。

キャプチャ時に`http.endpoint_patterns`が設定されている場合、レポートは記録された`path_pattern`でグループ化されます。レポートには入口レベルのエラー数と、トレースの`errors`からグループ化されたエラーサマリーも含まれます。

Markdownレポートの各パターンは`#### Representative Observed Flow`ブロックで終わり、そのパターンで最初に観測されたトレースの名前、`path (canonical)`と`path (concrete)`、`statement_normalized`形式のSQLリスト、および外部API呼び出しが示されます。`--value-mode raw`を明示し、かつキャプチャ時に`sql.capture_text`が有効だった場合だけ、正規化された形式の横に具体的なSQLも表示されます。
...
````md
# Observed Behavior Evidence Report

- Observed entrypoints: `1`

> Observed entrypoints are evidence views, not assumed migration units.

## POST /api/cart/add

- Entrypoint key: `POST /api/cart/add`
- Entrypoint type: `http`
- Migration unit assumption: `not_assumed`
...
PHPでは一部のケースで関数引数をcaptureできますが、php-ci3-unearthでは広範な自動引数captureには依存しません。
...
php-ci3-unearthはcaller file / lineの自動取得を行いません。CodeIgniter3のroute/controller情報と観測されたSQL/API patternを主な証拠として扱います。raw request bodyも無条件には保存しません。

## APM / OpenTelemetryとの関係

php-ci3-unearthはAPMやOpenTelemetryの概念と重なる部分がありますが、目的が違います。
...
* 観測された入口ごとの実行時挙動
* observed flow / behavior patternの発掘
* SQL pattern発掘
* 外部API呼び出し発掘
* 生成AIや人間が分析できるredacted observation data生成
* 業務仕様候補や移行対応関係を考えるための証拠データセット生成
* CodeIgniter3 controller pathと観測patternをつなげる
* CodeIgniter3からAIに渡しやすいlog schemaを固める
...
* observed entrypoint単位のJSON aggregate reportを生成できる
* observed entrypoint単位の簡易Markdown reportを生成できる
...
* php-ci3-unearth本体が仕様を断定生成するのではなく、仕様候補を作るための根拠を出す、という境界がREADMEで明確になっている
...
```bash
docker compose -f tests/e2e/codeigniter3-sqlite/docker-compose.yml -p php-ci3-unearth-sqlite exec -T --user www-data app php scripts/init-db.php
curl -i 'http://localhost:18080/api/products?category_id=1'
```

CodeIgniter3 hookの`params`は可変長引数として展開されず、hook methodへ1引数として渡されます。そのためfixtureでは`'params' => $unearthConfig`の形で設定配列を直接渡します。
