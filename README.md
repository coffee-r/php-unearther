...
php-unearthは、レガシーPHPアプリケーションの実行時の振る舞いを **言語非依存のJSONLログ** として記録し、人間や生成AIが安全に分析できる形へ変換するPHPライブラリを目指します。
...
汎用APMの代替ではありません。主目的は仕様発掘と、移行先実装でも同じformatのログを出して照合できる土台を作ることです。高度な分析、要約、pattern名付け、比較reportは生成AIや外部ツールに委ねられるので、php-unearth本体は **収集・正規化・マスキング・export** に寄せます。
