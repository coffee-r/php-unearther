...
このpackageは、endpointをそのまま移行単位だと決めつけません。旧WebAppでは、移行前の1 endpointが移行先で複数APIに分割されたり、複数endpointが1つのAPIへ統合されたり、一部の責務だけが別APIへ移ることがあります。php-unearthは、まず実行時に起きたSQL、外部API、request/response、view、errorを同じtraceで集め、後から業務仕様候補や移行対応関係を考えるための材料を作ります。
