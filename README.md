...
`failure_mode`は、php-unearth自体がリクエスト観測中に失敗した場合の動作を制御します。デフォルトは`throw`で、インストールや設定の問題がロールアウト中に可視化されます。本番環境で観測失敗時もアプリケーションを継続させたい場合は`log`に設定してください。`log`モードでは、CodeIgniter3は利用可能な場合`log_message('error', ...)`を使用し、それ以外は`error_log()`にフォールバックします。
