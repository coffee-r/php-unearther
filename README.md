...
また、CI3 Appでは `customer_no` のような同じ概念の値が、画面、API、DB、session、外部連携の境界で `string` / `number` / `null` として揺れることがあります。shapeは単発の型断定ではなく、観測された範囲で「このkeyはstringでもnumberでも来た」「nullableだった」という候補を集めるための材料として扱います。最終的な型や必須/任意の判断は、観測ログ、コード、DB定義、実シナリオを突き合わせて人間が確認します。
